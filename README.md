...
面向高校考研学子的微信小程序全栈应用，提供考研时间线追踪、学习打卡、资料共享、论坛社区、复试工具箱、数据大屏等功能，助力考研全流程。
...
│   │   └── admin.js                 # 后台管理
│   ├── middleware/
│   │   ├── auth.js                  # JWT 认证 (auth / optionalAuth / adminAuth)
...
共 30+ 张表，完整建表语句见 [database/schema.sql](database/schema.sql)。
...
| student_kaoyan_records | 学生考研信息录入表 |
| messages | 私信消息表 |
...
MYSQL_DATABASE // 默认 kaoyan_db
JWT_SECRET     // 默认 kaoyan-secret-key-2024
PORT           // 默认 3000
```
...
3. **连接池** — mysql2/promise 连接池 (上限 10 连接)，异步查询
4. **三层认证** — `auth` (必须登录) / `optionalAuth` (可选登录) / `adminAuth` (管理员)
