...
**校内考研助手**是一个专为高校考研学子打造的微信小程序全栈应用，覆盖考研备考全流程。系统提供考研时间线追踪、学习打卡、资料共享、论坛社区、复试工具箱、国家线查询、成绩估算、数据大屏等功能，并配套完善的管理后台。
...
| **缓存** | Redis + 内存缓存 | 三级 TTL（5 分钟 / 30 分钟 / 2 小时），自动降级 |
| **认证方案** | JWT 双 Token | Access Token（15 分钟）+ Refresh Token（7 天轮转），四级权限控制 |
| **日志系统** | Winston | 按日轮转日志文件，支持错误/组合/访问三类日志 |
| **文件处理** | Multer | MIME 类型校验 + 扩展名白名单 + 文件安全扫描 |
| **安全防护** | 自定义中间件 | 请求限流、安全响应头、XSS 过滤、敏感词过滤 |
...
- **考研时间线**：甘特图展示从预报名到待录取全流程节点，支持订阅提醒
...
- **国家线对比**：选择年份 / A区 B区 / 学硕专硕 / 学科门类，输入各科成绩
...
# 微信订阅消息模板ID（可选）
WECHAT_NOTIFICATION_TEMPLATE=
WECHAT_MESSAGE_TEMPLATE=

# AI 助手配置（可选，不配置使用默认回复）
AI_API_KEY=
...
│   ├── src/
│   │   ├── api/                      # API 请求封装
│   │   │   ├── request.js            # 请求拦截器、Token 刷新、弱网检测
│   │   │   └── index.js              # 接口方法集合
│   │   ├── components/               # 公共组件
│   │   │   ├── PageLoading/          # 页面加载动画组件
│   │   │   └── Skeleton/             # 骨架屏组件（首页/论坛/资料/列表）
│   │   ├── pages/                    # 页面模块
...
│   ├── app.js                        # 入口文件
│   ├── config/
│   │   ├── index.js                  # 配置中心（数据库/JWT/上传/微信/AI）
│   │   └── swagger.js                # Swagger 文档配置
│   ├── middleware/
│   │   ├── auth.js                   # 四级认证中间件
│   │   ├── cache.js                  # 缓存中间件（Redis + 内存双模式）
│   │   ├── security.js               # 安全中间件（限流/安全头/XSS 过滤）
│   │   └── upload.js                 # 文件上传中间件（Multer 配置）
│   ├── routes/
│   │   ├── user.js                   # 用户路由（登录/注册/资料/Token 刷新）
│   │   ├── home.js                   # 首页路由（倒计时/时间线/广告/通知）
│   │   ├── material.js               # 资料路由（分类/搜索/上传/下载/评价）
│   │   ├── forum.js                  # 论坛路由（帖子/评论/点赞/收藏）
│   │   ├── study.js                  # 学习路由（计划/打卡/统计/模板）
│   │   ├── admin.js                  # 管理路由（用户/审核/配置/统计/审计）
...
│   │   ├── init-db.js                # 自动建表与字段迁移
│   │   ├── response.js               # 统一响应格式
│   │   ├── sensitive-words.js        # 敏感词过滤（内存+数据库双模式）
│   │   ├── date.js                   # 日期工具
│   │   ├── logger.js                 # Winston 日志系统
│   │   ├── redis.js                  # Redis 客户端
│   │   ├── image-optimizer.js        # 图片优化工具
│   │   ├── file-scanner.js           # 文件安全扫描
│   │   ├── audit.js                  # 操作审计日志
│   │   ├── accountCleanup.js         # 账号注销定时清理
...
| 模块 | 路径前缀 | 主要功能 |
| --- | --- | --- |
| 用户 | `/api/user` | 微信登录、手机号登录、个人信息管理、Token 刷新、账号注销、用户资料页 |
...
| 消息 | `/api/message` | 私信会话、消息收发、用户屏蔽、对话删除 |
...
| 管理后台 | `/api/admin` | 用户管理、内容审核、举报处理、配置管理、统计总览、审计日志 |
| AI 助手 | `/api/ai` | 文本润色、口语练习 |
| 系统 | `/` | 测试接口、健康检查、缓存统计 |

> 完整 API 文档请启动后端后访问 `http://localhost:3000/api-docs`
...
6. **智能缓存** - Redis + 内存双模式缓存，三级 TTL 策略，Redis 不可用时自动降级
...
11. **Token 自动刷新** - 前端请求拦截器自动检测 Token 过期并刷新
12. **日志系统** - Winston 按日轮转，支持错误/组合/访问三级日志
...
确认 `server/.env` 中数据库配置正确，数据库 `kaoyan_db` 已创建。

### Redis 连接失败

Redis 为可选依赖。若未安装 Redis，系统会自动使用内存缓存，不影响功能。如需使用 Redis：

```bash
# Windows（使用 Windows Subsystem for Linux 或 Docker）
...
开发模式下可使用手机号直接登录，跳过微信授权流程。在登录页面输入手机号即可完成登录。微信小程序需在 `server/.env` 中配置 `WECHAT_APPID` 和 `WECHAT_APPSECRET`。

### 文件上传大小限制

默认限制 50MB，可在 `server/config/index.js` 中修改 `upload.maxFileSize`，同时调整 `server/app.js` 中 `express.json` 和 `express.urlencoded` 的 `limit` 参数。
...
建议配置 crontab 或 Windows 任务计划定时备份。
...
| 文档 | 说明 |
| --- | --- |
| [环境配置说明.md](环境配置说明.md) | 开发环境搭建详细教程 |
| [ERROR_FIX.md](ERROR_FIX.md) | 常见错误及解决方案 |
| [FIX_GUIDE.md](FIX_GUIDE.md) | 问题修复指南 |
| [OPTIMIZATION_GUIDE.md](OPTIMIZATION_GUIDE.md) | 性能优化建议与最佳实践 |
| [old_README.md](old_README.md) | 历史版本 README |

---

## Docker 部署（可选）

可通过 Docker Compose 一键启动项目所需的基础服务（MySQL + Redis），适合本地开发或快速体验。
...
- **首页模块**：考研倒计时、时间线甘特图、励志文案、轮播广告、滚动通知、快捷入口
- **资料中心**：多级分类、多维筛选、搜索排序、文件上传/下载（水印保护）、评价系统、收藏管理、审核机制
- **论坛社区**：五大板块、富文本发帖、匿名发布、点赞/评论/收藏/举报、内容审核
- **学习打卡**：学习计划 CRUD、四套模板、任务打卡、打卡日历、统计看板、心情记录
- **复试工具箱**：英语口语题库、复试简历模板、导师联系邮件模板
- **成绩估算器**：国家线对比、逐科分析、六档综合评估、个性化备考建议
- **国家线查询**：多维筛选、多年度对比、后台管理
- **学校官网导航**：地区/类型筛选、一键跳转、点击统计
- **头衔认证**：上岸认证申请与审核
- **私信聊天**：一对一实时对话、防骚扰机制、用户屏蔽、举报与禁言
- **通知公告**：多类型通知、置顶强提醒、已读追踪
- **意见反馈**：用户提交与管理端处理
- **举报系统**：多类型举报、状态流转、处理记录
- **个人中心**：个人资料、数据统计、功能入口、消息中心
- **管理后台**：数据概览、用户管理、内容审核、举报处理、国家线管理、通知公告、广告管理、数据大屏、学校官网、反馈处理、头衔审核、审计日志、系统配置
- **AI 助手**：文本润色、口语练习
- **安全体系**：JWT 双 Token、四级权限、请求限流、XSS 防护、敏感词过滤、操作审计、文件安全扫描
...
2. **编码规范**
   - 前端：遵循 Vue 3 Composition API + UniApp 最佳实践
   - 后端：遵循 Express 中间件模式，路由模块化
   - 命名：使用驼峰命名（camelCase），常量使用大写字母
   - 注释：关键逻辑需添加注释说明
3. **Commit 规范**
